    ...
    <div class="catalog-section">
        ...
        <div class="category-tabs" style="display: flex; gap: 10px; overflow-x: auto; padding-bottom: 15px; margin-bottom: 20px;">
            <a href="{{ route('pos.index') }}" 
               ...
               style="padding: 10px 20px; border-radius: 100px; background: {{ !request('category_id') ? 'var(--primary)' : 'white' }}; color: {{ !request('category_id') ? 'white' : 'var(--text-main)' }}; text-decoration: none; font-size: 13px; font-weight: 700; border: 1px solid {{ !request('category_id') ? 'var(--primary)' : 'var(--border)' }}; white-space: nowrap; transition: var(--transition);">
               Semua Menu
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('pos.index', ['category_id' => $cat->id]) }}" 
               class="category-tab {{ request('category_id') == $cat->id ? 'active' : '' }}"
               style="padding: 10px 20px; border-radius: 100px; background: {{ request('category_id') == $cat->id ? 'var(--primary)' : 'white' }}; color: {{ request('category_id') == $cat->id ? 'white' : 'var(--text-main)' }}; text-decoration: none; font-size: 13px; font-weight: 700; border: 1px solid {{ request('category_id') == $cat->id ? 'var(--primary)' : 'var(--border)' }}; white-space: nowrap; transition: var(--transition);">
               {{ $cat->name }}
            </a>
            @endforeach
        </div>
    ...
    function submitTransaksi() {
        ...
        .then(result => {
            if(result.success) {
                ...
                var printUrl = "{{ url('transactions') }}/" + result.transaction_id + "/print";
